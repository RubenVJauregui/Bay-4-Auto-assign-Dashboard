/**
 * Bay 4 Assignments — Authoritative Operational Data
 * Valley View Warehouse (LT_F1), DOCK50–DOCK72
 *
 * TASK DATA: Refreshed 2026-07-13 ~11:17 PDT (live WISE/WMS APIs)
 *   Sources:
 *     - /wms-bam/outbound/load-task/search — active load tasks
 *     - /wms-bam/inbound/receive-task/search — active receive tasks (incl. assigneeUserName)
 *     - Assignee mapping resolved per-task via load-task + receive-task APIs
 *
 * Do NOT fabricate, estimate, or guess any metric.
 */

export type DoorStatus = "Occupied" | "Reserved" | "Available";

export interface DoorRecord {
  door: string;
  status: DoorStatus;
  assignee: string | null;
  customer: string | null;
  taskIds: string[];
  duration: string | null;
  anomaly: boolean;
}

export interface KpiMetric {
  label: string;
  value: string;
  numerator: number;
  denominator: number;
  percentage: number;
}

export interface AssigneeSummary {
  name: string;
  taskCount: number;
}

export interface MixMetric {
  label: string;
  count: number;
  total: number;
}

export interface TaskRecord {
  taskId: string;
  dns: string;
  customer: string;
  pieces: string;
  assignee: string;
  door: string;
}

export const TOTAL_DOORS = 23;

export const doors: DoorRecord[] = [
  // ═══════════════════════════════════════════════════════════════
  // OCCUPIED — doors with IN_PROGRESS tasks (7 doors)
  // ═══════════════════════════════════════════════════════════════
  {
    door: "DOCK50",
    status: "Occupied",
    assignee: "ARNULFO MUNGUIA / Fatima Ponce",
    customer: "KARAKA, LLC / GURUNANDA, LLC",
    taskIds: ["TASK-5313226", "TASK-5314531", "TASK-5314641"],
    duration: "~2.8d",
    anomaly: false,
  },
  {
    door: "DOCK52",
    status: "Occupied",
    assignee: "DANIEL BELTRAN",
    customer: "GURUNANDA, LLC",
    taskIds: ["TASK-5313819"],
    duration: "~2.6d",
    anomaly: false,
  },
  {
    door: "DOCK56",
    status: "Occupied",
    assignee: "Caren Cubides",
    customer: "GURUNANDA, LLC",
    taskIds: ["TASK-5314475"],
    duration: "<1d",
    anomaly: false,
  },
  {
    door: "DOCK58",
    status: "Occupied",
    assignee: "DANIEL BELTRAN",
    customer: "ORGAIN, LLC.",
    taskIds: ["TASK-5314473"],
    duration: "<1d",
    anomaly: false,
  },
  {
    door: "DOCK59",
    status: "Occupied",
    assignee: "Caren Cubides",
    customer: "GURUNANDA, LLC",
    taskIds: ["TASK-5314468"],
    duration: "<1d",
    anomaly: false,
  },
  {
    door: "DOCK66",
    status: "Occupied",
    assignee: "DANIELA GONZALEZ / DANIEL BELTRAN",
    customer: "GURUNANDA, LLC",
    taskIds: ["TASK-5313877", "TASK-5314633"],
    duration: "~2.3d",
    anomaly: false,
  },
  {
    door: "DOCK68",
    status: "Occupied",
    assignee: "Caren Cubides",
    customer: "GURUNANDA, LLC",
    taskIds: ["TASK-5313647"],
    duration: "~2.6d",
    anomaly: false,
  },

  // ═══════════════════════════════════════════════════════════════
  // RESERVED — doors with only NEW tasks (4 doors)
  // ═══════════════════════════════════════════════════════════════
  {
    door: "DOCK51",
    status: "Reserved",
    assignee: "ARNULFO MUNGUIA",
    customer: "KARAKA, LLC / GURUNANDA, LLC",
    taskIds: ["TASK-5313444", "TASK-5314679"],
    duration: "~2.7d",
    anomaly: true,
  },
  {
    door: "DOCK53",
    status: "Reserved",
    assignee: "ARNULFO MUNGUIA",
    customer: "GURUNANDA, LLC",
    taskIds: ["TASK-5314634"],
    duration: "<1d",
    anomaly: true,
  },
  {
    door: "DOCK55",
    status: "Reserved",
    assignee: "Nanci Viviana Rosas",
    customer: "KARAKA, LLC",
    taskIds: ["TASK-5314391"],
    duration: "<1d",
    anomaly: true,
  },
  {
    door: "DOCK57",
    status: "Reserved",
    assignee: "RUFINO MUNGUIA",
    customer: "GURUNANDA, LLC",
    taskIds: ["TASK-5314517"],
    duration: "<1d",
    anomaly: true,
  },

  // ═══════════════════════════════════════════════════════════════
  // AVAILABLE — no active tasks (12 doors)
  // ═══════════════════════════════════════════════════════════════
  { door: "DOCK54", status: "Available", assignee: null, customer: null, taskIds: [], duration: null, anomaly: false },
  { door: "DOCK60", status: "Available", assignee: null, customer: null, taskIds: [], duration: null, anomaly: false },
  { door: "DOCK61", status: "Available", assignee: null, customer: null, taskIds: [], duration: null, anomaly: false },
  { door: "DOCK62", status: "Available", assignee: null, customer: null, taskIds: [], duration: null, anomaly: false },
  { door: "DOCK63", status: "Available", assignee: null, customer: null, taskIds: [], duration: null, anomaly: false },
  { door: "DOCK64", status: "Available", assignee: null, customer: null, taskIds: [], duration: null, anomaly: false },
  { door: "DOCK65", status: "Available", assignee: null, customer: null, taskIds: [], duration: null, anomaly: false },
  { door: "DOCK67", status: "Available", assignee: null, customer: null, taskIds: [], duration: null, anomaly: false },
  { door: "DOCK69", status: "Available", assignee: null, customer: null, taskIds: [], duration: null, anomaly: false },
  { door: "DOCK70", status: "Available", assignee: null, customer: null, taskIds: [], duration: null, anomaly: false },
  { door: "DOCK71", status: "Available", assignee: null, customer: null, taskIds: [], duration: null, anomaly: false },
  { door: "DOCK72", status: "Available", assignee: null, customer: null, taskIds: [], duration: null, anomaly: false },
];

const occupied = doors.filter((d) => d.status === "Occupied").length;
const reserved = doors.filter((d) => d.status === "Reserved").length;
const available = doors.filter((d) => d.status === "Available").length;
const doorsWithTasks = doors.filter((d) => d.taskIds.length > 0).length;

export const kpiMetrics: KpiMetric[] = [
  {
    label: "Doors w/ Active Tasks",
    value: `${doorsWithTasks}/23`,
    numerator: doorsWithTasks,
    denominator: TOTAL_DOORS,
    percentage: (doorsWithTasks / TOTAL_DOORS) * 100,
  },
  {
    label: "In Progress (Occupied)",
    value: `${occupied}`,
    numerator: occupied,
    denominator: TOTAL_DOORS,
    percentage: (occupied / TOTAL_DOORS) * 100,
  },
  {
    label: "Doors Available",
    value: `${available}`,
    numerator: available,
    denominator: TOTAL_DOORS,
    percentage: (available / TOTAL_DOORS) * 100,
  },
  {
    label: "Task Occupancy Rate",
    value: `${((doorsWithTasks / TOTAL_DOORS) * 100).toFixed(1)}%`,
    numerator: doorsWithTasks,
    denominator: TOTAL_DOORS,
    percentage: (doorsWithTasks / TOTAL_DOORS) * 100,
  },
];

// Active assignee task counts — based on Bay 4 DOCK50-DOCK72 task-level assignee mapping
export const assigneeSummaries: AssigneeSummary[] = [
  { name: "ARNULFO MUNGUIA", taskCount: 5 },
  { name: "DANIEL BELTRAN", taskCount: 3 },
  { name: "Caren Cubides", taskCount: 3 },
  { name: "DANIELA GONZALEZ", taskCount: 1 },
  { name: "Fatima Del Rosario Ponce", taskCount: 1 },
  { name: "Nanci Viviana Rosas", taskCount: 1 },
  { name: "RUFINO MUNGUIA", taskCount: 1 },
];

// All-time assignment counts — preserved from prior baseline
export const allTimeAssigneeSummaries: AssigneeSummary[] = [
  { name: "Arnulfo Munguia (89)", taskCount: 110 },
  { name: "Daniel Beltran", taskCount: 90 },
  { name: "Caren Cubides", taskCount: 3 },
  { name: "Daniela Gonzalez", taskCount: 1 },
  { name: "Fatima Ponce", taskCount: 1 },
  { name: "Nanci Viviana Rosas", taskCount: 1 },
  { name: "Rufino Munguia", taskCount: 1 },
];

// Mix: 6 LOAD (outbound) + 9 RECEIVE (inbound) = 15 active tasks at Bay 4 doors
export const inboundOutboundMix: MixMetric[] = [
  { label: "Outbound", count: 6, total: 15 },
  { label: "Inbound", count: 9, total: 15 },
];

export const activeInboundOutboundMix: MixMetric[] = [
  { label: "Outbound", count: 6, total: 15 },
  { label: "Inbound", count: 9, total: 15 },
];

// Schedule: Appointment API returned 400 for all parameter combinations.
// % scheduled inbounds received and % scheduled outbounds loaded UNAVAILABLE.
export const scheduleAvailable = false;
export const scheduledInboundOrders = 0;
export const scheduledOutboundOrders = 0;
export const scheduledInboundReceived = 0;
export const scheduledOutboundLoaded = 0;
export const pctScheduledInboundReceived = 0;
export const pctScheduledOutboundLoaded = 0;

// Facility-wide appointment context — unavailable
export const facilityWideReceiptsCreated = 0;
export const facilityWideReceiptsReceived = 0;
export const facilityWideLoadsCreated = 0;
export const facilityWideLoadsShipped = 0;

// Door occupancy duration: available from task startTime
export const doorDurationsAvailable = true;

// Active task records from fresh WISE data (July 13, 2026)
// 15 active tasks: 6 LOAD (outbound) + 9 RECEIVE (inbound)
export const assignments: TaskRecord[] = [
  // ────── OUTBOUND / LOAD — IN_PROGRESS (3) ──────
  {
    taskId: "TASK-5313819",
    dns: "LOAD",
    customer: "GURUNANDA, LLC",
    pieces: "IN_PROGRESS (~2.6d)",
    assignee: "DANIEL BELTRAN",
    door: "DOCK52",
  },
  {
    taskId: "TASK-5314473",
    dns: "LOAD",
    customer: "ORGAIN, LLC.",
    pieces: "IN_PROGRESS (<1d)",
    assignee: "DANIEL BELTRAN",
    door: "DOCK58",
  },

  // ────── OUTBOUND / LOAD — NEW (3) ──────
  {
    taskId: "TASK-5314679",
    dns: "LOAD",
    customer: "GURUNANDA, LLC",
    pieces: "NEW (<1d)",
    assignee: "ARNULFO MUNGUIA",
    door: "DOCK51",
  },
  {
    taskId: "TASK-5314634",
    dns: "LOAD",
    customer: "GURUNANDA, LLC",
    pieces: "NEW (<1d)",
    assignee: "ARNULFO MUNGUIA",
    door: "DOCK53",
  },
  {
    taskId: "TASK-5314391",
    dns: "LOAD",
    customer: "KARAKA, LLC",
    pieces: "NEW (<1d)",
    assignee: "Nanci Viviana Rosas",
    door: "DOCK55",
  },
  {
    taskId: "TASK-5314633",
    dns: "LOAD",
    customer: "GURUNANDA, LLC",
    pieces: "NEW (<1d)",
    assignee: "DANIEL BELTRAN",
    door: "DOCK66",
  },

  // ────── INBOUND / RECEIVE — IN_PROGRESS (6) ──────
  {
    taskId: "TASK-5314531",
    dns: "RECEIVE",
    customer: "KARAKA, LLC",
    pieces: "IN_PROGRESS (<1d)",
    assignee: "ARNULFO MUNGUIA",
    door: "DOCK50",
  },
  {
    taskId: "TASK-5314641",
    dns: "RECEIVE",
    customer: "GURUNANDA, LLC",
    pieces: "IN_PROGRESS (<1d)",
    assignee: "Fatima Del Rosario Ponce",
    door: "DOCK50",
  },
  {
    taskId: "TASK-5314475",
    dns: "RECEIVE",
    customer: "GURUNANDA, LLC",
    pieces: "IN_PROGRESS (<1d)",
    assignee: "Caren Cubides",
    door: "DOCK56",
  },
  {
    taskId: "TASK-5314468",
    dns: "RECEIVE",
    customer: "GURUNANDA, LLC",
    pieces: "IN_PROGRESS (<1d)",
    assignee: "Caren Cubides",
    door: "DOCK59",
  },
  {
    taskId: "TASK-5313877",
    dns: "RECEIVE",
    customer: "GURUNANDA, LLC",
    pieces: "IN_PROGRESS (~2.3d)",
    assignee: "DANIELA GONZALEZ",
    door: "DOCK66",
  },
  {
    taskId: "TASK-5313647",
    dns: "RECEIVE",
    customer: "GURUNANDA, LLC",
    pieces: "IN_PROGRESS (~2.6d)",
    assignee: "Caren Cubides",
    door: "DOCK68",
  },

  // ────── INBOUND / RECEIVE — NEW (3) ──────
  {
    taskId: "TASK-5313226",
    dns: "RECEIVE",
    customer: "KARAKA, LLC",
    pieces: "NEW (~2.8d)",
    assignee: "ARNULFO MUNGUIA",
    door: "DOCK50",
  },
  {
    taskId: "TASK-5313444",
    dns: "RECEIVE",
    customer: "KARAKA, LLC",
    pieces: "NEW (~2.7d)",
    assignee: "ARNULFO MUNGUIA",
    door: "DOCK51",
  },
  {
    taskId: "TASK-5314517",
    dns: "RECEIVE",
    customer: "GURUNANDA, LLC",
    pieces: "NEW (<1d)",
    assignee: "RUFINO MUNGUIA",
    door: "DOCK57",
  },
];
